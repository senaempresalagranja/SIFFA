...
// Con el siguiente bloque de código podemos escribir en la casilla que deseamos, es muy sencillo el manejo tanto para hacerlo manualmente como dinámicamente.
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('B7', 'Latitud')
            ->setCellValue('C7', 'Longitud')
            ->setCellValue('D7', 'Tamaño Lote (Ha)')
            ->setCellValue('E7', 'Finca')
            ->setCellValue('F7', 'Tamaño Finca')
            ->setCellValue('G7', 'Vereda')
            ->setCellValue('H7', 'Municipio')
            ->setCellValue('I7', 'Departamento');
...
// Luego de escribir en la hoja de cálculo pasamos a darle un nombre y definir con que hoja abrirá el documento, en este caso como tenemos solo uno, el valor será “0”.
            ...
            $objPHPExcel->getActiveSheet()->setTitle('Lotes_SIFFA');
$objPHPExcel->setActiveSheetIndex(0);


$connection=mysqli_connect("localhost","root","","siffa");
$query="SELECT NumeroLote,latitud,longitud, Ha,Nombre,NombreVereda, Nombre_Municipio, NombreDepartamento,Tamano FROM `lote`  INNER JOIN Hacienda ON lote.IdHacienda=hacienda.Idhacienda INNER JOIN vereda ON hacienda.Idvereda=vereda.Idvereda INNER JOIN municipio ON vereda.Idmunicipio=municipio.Idmunicipio INNER JOIN departamento ON municipio.Iddepartamento=departamento.Iddepartamento  ORDER BY Nombre,NumeroLote";
...
while ($fila=mysqli_fetch_row($resource)) {
    ...
    $objPHPExcel->setActiveSheetIndex(0)
     ...
     $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('G'.$y, $fila[5])
            ->setCellValue('H'.$y, $fila[6])
...
header('Content-Disposition: attachment;filename="Lotes_SIFFA.xls"');
